...
namespace RulerBox
{
    public class BuildingPlacementTool : MonoBehaviour
    {
        ...
        public static bool CheckAffordability(City city, InvestmentsList.InvestmentDefinition def)
        {
            ...
            // 2. Check Treasury (RulerBox Economy)
            // Even though it's a percentage, we ensure they have at least 1 gold if cost > 0? 
            // Usually percentage checks always pass unless treasury is 0.
            // We'll skip strict blocking for treasury to be lenient, or you can add it here.
        ...
        private void DeductCosts(City city, InvestmentsList.InvestmentDefinition def)
        {
            ...
            // 2. Deduct Hidden Treasury Cost (RulerBox Economy)
            if (def.TreasuryPctCost > 0f && city.kingdom != null)
            {
                ...
                if (d != null && d.Treasury > 0)
                {
                    ...
                    {
                        d.Treasury -= tax;
                        // Optional: Show floating text for gold loss?
